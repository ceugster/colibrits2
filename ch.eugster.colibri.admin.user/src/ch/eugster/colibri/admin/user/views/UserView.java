...
public class UserView extends AbstractEntityView implements IDoubleClickListener
{
	public static final String ID = "ch.eugster.colibri.admin.user.view";
	...
	public void createPartControl(final Composite parent)
	{
		...
		this.viewer.setContentProvider(new UserContentProvider());
		...
		tableViewerColumn.setLabelProvider(new CellLabelProvider()
		{
			...
			public void update(final ViewerCell cell)
			{
				...
				if (object instanceof User)
				{
					...
					{
						cell.setImage(Activator.getDefault().getImageRegistry().get(Activator.USER_RED));
					}
					else
					{
						cell.setImage(Activator.getDefault().getImageRegistry().get(Activator.USER_BLUE));
					}
				}
			}
		});
		...
		tableViewerColumn.setLabelProvider(new CellLabelProvider()
		{
			...
			public void update(final ViewerCell cell)
			{
				...
				{
					final User user = (User) object;
					cell.setText(user.getRole().getName());
				}
			}
		});
				...
				PersistenceService.class, null)
		{
			...
			public PersistenceService addingService(final ServiceReference<PersistenceService> reference)
			{
				...
				final UIJob job = new UIJob("Lade Benutzer...")
				{

					@Override
					public IStatus runInUIThread(final IProgressMonitor monitor)
					{
						UserView.this.viewer.setInput(service);
						UserView.this.packColumns();
			...
			public void removedService(final ServiceReference<PersistenceService> reference, final PersistenceService service)
			{
				...
				final UIJob job = new UIJob("Lade Benutzer...")
				{
					...
					public IStatus runInUIThread(final IProgressMonitor monitor)
					{
						if (UserView.this.viewer.getContentProvider() != null)
						{
							UserView.this.viewer.setInput(UserView.this.viewer);
							UserView.this.packColumns();
						}
						return Status.OK_STATUS;
					}
	...
	public void doubleClick(final DoubleClickEvent event)
	{
		...
		{
			Activator.getDefault().editUser((User) object);
		}
		else
		{
			return;

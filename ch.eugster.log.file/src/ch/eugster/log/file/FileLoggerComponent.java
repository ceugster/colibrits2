...
public class FileLoggerComponent implements LogListener
{
						...
						File[] oldLogFiles = history.listFiles(new FileFilter() 
						{
							...
							public boolean accept(File file) 
							{
								if (file.getName().startsWith("log_") && file.getName().length() >= 12)
								{
									...
									Calendar todayMinusDays = GregorianCalendar.getInstance(Locale.getDefault());
									todayMinusDays.add(Calendar.DATE, -days);
									Calendar fileDate = GregorianCalendar.getInstance(Locale.getDefault());
	...
	public void logged(LogEntry entry)
	{
		if (out != null || Activator.getDefault() != null)
		{
			...
			{
				String log = SimpleDateFormat.getDateTimeInstance().format(GregorianCalendar.getInstance(Locale.getDefault()).getTime()) + " " + String.format("[%s] <%s> %s", Activator.getDefault().getLevelAsString(entry.getLevel()), entry.getBundle().getSymbolicName(), entry.getMessage());
				out.println(log);

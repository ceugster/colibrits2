...
public class EditSalespointHandler extends AbstractPersistenceClientHandler
{
	@Override
	public Object execute(final ExecutionEvent event) throws ExecutionException
	{
		if (event.getApplicationContext() instanceof EvaluationContext)
		{
			...
			if (object instanceof StructuredSelection)
			{
				...
				if ((ssel.size() == 1) && (ssel.getFirstElement() instanceof Salespoint))
				{
					Activator.getDefault().editSalespoint((Salespoint) ssel.getFirstElement());
				}
			}
		}
		return null;
	...
	public void setEnabled(EvaluationContext context)
	{
		...
		{
			final StructuredSelection ssel = (StructuredSelection) object;
			setBaseEnabled(ssel.size() == 1 && ssel.getFirstElement() instanceof Salespoint);
		}
	}

		...
		if (event.getApplicationContext() instanceof EvaluationContext)
		{
			...
			if (object instanceof StructuredSelection)
			{
				...
				if (!ssel.isEmpty())
				{
					...
					if (ssel.size() == 1)
					{
						msg = "Soll die ausgewählte Währung:\n" + sb.toString() + " entfernt werden?";
					}
					else
					{
						msg = "Sollen die ausgewählten Währungen:\n" + sb.toString() + " entfernt werden?";
					}

					final Shell shell = (Shell) ctx.getParent().getVariable("activeShell"); //$NON-NLS-1$
					final MessageDialog dialog = new MessageDialog(shell, "Währung entfernen", null, msg, MessageDialog.QUESTION, new String[] {
							"Ja", "Nein" }, 0);

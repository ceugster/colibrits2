...
class ViewerCanvas extends Canvas
{
	...
	ViewerCanvas(final Composite parent, final int style)
	{
	 ...
	 * @return the calculated size
	 */
	public Point getFitSize(final int contentWidth, final int contentHeight)
	{
		Rectangle bounds = getBounds();
		int borderWidth = getBorderWidth();
		Point size = new Point(bounds.width - 2 * borderWidth - 2 * MARGIN, bounds.height - 2 * borderWidth - 2
				* MARGIN);
	...
	private JRPrintHyperlink getHyperlinkAt(final int x, final int y)
	{
			...
			for (Iterator<JRPrintElement> it = elements.iterator(); it.hasNext();)
			{
				...
				if (getImageMapRenderer(element) != null)
				{
					...
					if (hyperlinks != null)
						...
						&& ((JRPrintHyperlink) element).getHyperlinkTypeValue() != HyperlinkTypeEnum.NONE)
				{
	...
	private boolean needHScroll(final int contentWidth, final boolean vVisible)
	{
	...
	private boolean needVScroll(final int contentHeight, final boolean hVisible)
	{
		ScrollBar vBar = getVerticalBar();
		if (vBar == null)
			return false;
	...
	private void notifyHyperlinkListeners(final JRPrintHyperlink link)
	{
	...
	protected void refresh()
	{
		...
		else if (viewer.hasDocument())
		{
			try
			{
				...
				refresh(image, null, viewer.getDocument().getPages().get(viewer.getPageIndex()));
			}
			catch (JRException e)
			{
	...
	private void refresh(final Image reportImage, final String errorMessage, final JRPrintPage page)
	{
		if (isDisposed())
			return;
	...
	private void setHorizontalSelection(final int selection)
	{
		ScrollBar sb = getHorizontalBar();
	...
	private void setVerticalSelection(final int selection)
	{
		ScrollBar sb = getVerticalBar();
		sb.setSelection(Math.max(sb.getMinimum(), Math.min(selection, sb.getMaximum())));
	}

	private Point translateRelativeToPage(final int x, final int y)
	{
		Rectangle bounds = reportImage.getBounds();
		Rectangle clientArea = getClientArea();
	...
	private void updateHyperlink(final int x, final int y)
	{
	...
	private static ImageMapRenderable getImageMapRenderer(final JRPrintElement element)
	{
	...
	private static class ImageAreaHyperlink implements IHyperlinkContainer
	{
		...
		public ImageAreaHyperlink(final int originX, final int originY, final List<JRPrintImageAreaHyperlink> imageAreaHyperlinks)
		{
		...
		@Override
		public JRPrintHyperlink getHyperlink(final Point point)
		{
	...
	private static class PrintHyperlink implements IHyperlinkContainer
	{
		...
		public PrintHyperlink(final int originX, final int originY, final JRPrintElement element)
		{
		...
		private boolean inside(final Point point)
		{

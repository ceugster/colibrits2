...
{
	public static final String ID = "ch.eugster.colibri.admin.layout.editor.printer";
	...
	private final Map<ILayoutSectionType, Button> salespointSpecifics = new HashMap<ILayoutSectionType, Button>();
	...
	protected void saveValues()
	{
		...
		for (final ILayoutSectionType layoutSectionType : this.layoutType.getLayoutSectionTypes())
		{
			...
			else
			{
				 ...
				 * salespoint specific
				 */
				if (this.salespointSpecifics.get(layoutSectionType).getSelection())
				{
					if ((printoutArea == null))
					{
						printoutArea = PrintoutArea.newInstance(printout, layoutSectionType.ordinal());
					...
					StyledText pattern = this.getStyledText(layoutSectionType, AreaType.TITLE);
					if (pattern != null)
					{
						printoutArea.setTitlePattern(pattern.getText());
					}
					pattern = this.getStyledText(layoutSectionType, AreaType.DETAIL);
					if (pattern != null)
					{
						printoutArea.setDetailPattern(pattern.getText());
					}
					pattern = this.getStyledText(layoutSectionType, AreaType.TOTAL);
					if (pattern != null)
					{
						printoutArea.setTotalPattern(pattern.getText());
					}
					this.savePatterns(layoutSectionType, printoutArea);
					this.savePrintOptions(layoutSectionType, printoutArea);
				}
	...
	private Control fillAreaSection(final Section parent, final ILayoutSectionType layoutSectionType)
	{
		...
		if (printout.isSalespointSpecific())
		{
			...
			button.addSelectionListener(new SelectionListener()
			{
				...
				public void widgetSelected(final SelectionEvent e)
				{
					...
					PrintoutEditor.this.setWidgetsEnabled(layoutSectionType, enabled);
					PrintoutEditor.this.setDirty(true);
				}
			});
			this.salespointSpecifics.put(layoutSectionType, button);
		}
	...
	private void loadPrintoutAreaValues(final PrintoutArea printoutArea)
	{
		...
		if (!this.salespointSpecifics.isEmpty())
		{
			final boolean enabled = !printoutArea.isDeleted() && printoutArea.getPrintout().isSalespointSpecific();
			this.salespointSpecifics.get(layoutSectionType).setSelection(enabled);
			this.setWidgetsEnabled(layoutSectionType, enabled);
		}
	...
	private void setPatternWidgetsEnabled(final ILayoutSectionType layoutSectionType, final boolean enabled,
			final Color color)
	{
		for (final AreaType areaType : AreaType.values())
		{
			if (layoutSectionType.getLayoutSection().hasArea(areaType))
			{
				...
				pattern.setBackground(color);
			}
		}
	}

	private void setPrintOptionWidgetsEnabled(final ILayoutSectionType layoutSectionType, final boolean enabled)
	{
		for (final AreaType areaType : AreaType.values())
		{
			if (layoutSectionType.getLayoutSection().hasArea(areaType))
	...
	private void setWidgetsEnabled(final ILayoutSectionType layoutAreaType, final boolean enabled)
	{
		final Color color = enabled ? Display.getCurrent().getSystemColor(SWT.COLOR_WHITE) : Display.getCurrent()
				.getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW);
		this.setPrintOptionWidgetsEnabled(layoutAreaType, enabled);
		this.setPatternWidgetsEnabled(layoutAreaType, enabled, color);
	}
}

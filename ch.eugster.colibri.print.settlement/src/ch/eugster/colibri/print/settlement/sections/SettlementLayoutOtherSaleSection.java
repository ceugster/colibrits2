...
public class SettlementLayoutOtherSaleSection extends AbstractLayoutSection
{
	...
	public SettlementLayoutOtherSaleSection(final ILayoutSectionType layoutAreaType)
	{
	...
	public String getDefaultPatternTotal()
	{
		StringBuilder builder = new StringBuilder();
		builder = builder.append("------------------------------------------\n");
		builder = builder.append("Sonstige Verkäufe  MMMM BBBBBBBBBB TTTTTTT\n");
		builder = builder.append("==========================================");
		return builder.toString();
	}
	...
	protected Collection<String> prepareAreaDetail(final IPrintable printable)
	{
		...
		if (printIt(AreaType.DETAIL, printable))
		{
			if (printable instanceof Settlement)
			{
				...
				{
					if (settlementPosition.getProductGroupType().equals(ProductGroupType.NON_SALES_RELATED))
					{
		...
		if (printIt(AreaType.TOTAL, printable))
		{
			if (printable instanceof Settlement)
			{
				...
				if (!settlementPositions.isEmpty())
				{
					...
					{
						if (position.getProductGroupType().equals(ProductGroupType.NON_SALES_RELATED))
						{
		...
		public String replace(final ILayoutSection layoutArea, final IPrintable printable, final String marker)
		{
			if (printable instanceof SettlementPosition)
			{
				...
				switch (this)
				{
					...
					case M:
					{
						final String quantity = SettlementLayoutOtherSaleSection.quantityFormatter.format(detail
								.getQuantity());
					...
					case B:
					{
						...
						SettlementLayoutOtherSaleSection.amountFormatter.setMinimumFractionDigits(currency
								.getDefaultFractionDigits());
						SettlementLayoutOtherSaleSection.amountFormatter.setMaximumFractionDigits(currency
								.getDefaultFractionDigits());
						amountFormatter.setGroupingUsed(false);
						final String amount = SettlementLayoutOtherSaleSection.amountFormatter.format(detail
					...
					case T:
					{
						...
						return layoutArea.replaceMarker(
								SettlementLayoutOtherSaleSection.amountFormatter.format(taxAmount), marker, false);
					}
					default:
					{
						throw new RuntimeException("invalid key");
			...
			if (printable instanceof SettlementPosition)
			{
				...
				switch (this)
				{
					...
					case B:
					{
						...
						SettlementLayoutOtherSaleSection.amountFormatter.setMaximumFractionDigits(currency
								.getDefaultFractionDigits());
						String amount = SettlementLayoutOtherSaleSection.amountFormatter.format(detail
								.getDefaultCurrencyAmount());
						return layoutArea.replaceMarker(amount, marker, false);
					}
					case T:
